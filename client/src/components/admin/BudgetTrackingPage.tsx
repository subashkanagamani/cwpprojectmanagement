...
import { Plus, Edit2, Trash2, DollarSign, Wallet, Users } from 'lucide-react';
    ...
    <div className="space-y-8">
      <div className="animate-fade-up flex justify-between items-center gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage client budgets and spending</p>
        </div>
      ...
      <div className="animate-fade-up grid grid-cols-1 md:grid-cols-3 gap-6" style={{ animationDelay: "100ms" }}>
        ...
        <Card>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {budgets.map((budget) => (
                  <TableRow key={budget.id} data-testid={`row-budget-${budget.id}`} className="hover:bg-muted/50 transition-colors cursor-pointer group">
                ...
                {budgets.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-12">
                      <DollarSign className="h-8 w-8 opacity-40 mx-auto mb-2" />
                      <p className="text-sm text-muted-foreground">No budgets configured yet. Add your first budget to get started.</p>
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
